...
load("data/GO_MPK_TE.rda")
TE_d_tmp <- flattenCorrMatrix(GO_MPK_TE)
TE_d <- TE_d_tmp %>% filter(cor!=1)

load("data/GO_MPK_RNA.rda")
RNA_d_tmp <- flattenCorrMatrix(GO_MPK_RNA)
...
net_all_leaves <- colnames(GO_MPK_TE)
